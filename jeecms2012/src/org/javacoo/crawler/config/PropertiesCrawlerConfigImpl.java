package org.javacoo.crawler.config;
...
import org.javacoo.crawler.util.PropertiesUtil;
...
public class PropertiesCrawlerConfigImpl implements ICrawlerConfig{
	...
	public void loadCrawlerConfig() {
		...
		if(!isBlank(properties.getProperty("replaceName"))){
			CrawlerConfig.replaceName = properties.getProperty("replaceName");
			log.info("已加载配置：是否重命名:" + CrawlerConfig.replaceName);
		}else{
			log.info("未配置：是否重命名，采用系统默认:" + CrawlerConfig.replaceName);
		}
