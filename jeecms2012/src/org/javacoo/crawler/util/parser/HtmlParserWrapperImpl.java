package org.javacoo.crawler.util.parser;
...
public class HtmlParserWrapperImpl implements HtmlParserWrapper{
	...
	public List<CrawlLinkURI> getLinkAreaUrlList(String orginHtml,CrawlURI parentCrawlURI) {
		...
		if(!CollectionUtils.isEmpty(linkAreafilter.getFetchAreaTagMap())){
	 ...
	 * @return 字段 值对照MAP
	 */
	public Map<String,List<String>> getFieldValues(String orginHtml){
		log.info("========取得原始HTML中指定条件的值并放在对应字段=========");
		Map<String,List<String>> resultMap = new HashMap<String, List<String>>();
		Filter<String,Map<String, String>> fieldfilter = filterFactory.getFilterByName(Constants.FILTER_NAME_FIELD);
		String field = null;
		...
		for(Iterator<String> it = fieldfilter.getFetchAreaTagMap().keySet().iterator(); it.hasNext();){
			...
			resultMap.put(field, htmlParser.getPlainTextList(tempHtml, fieldfilter.getFetchAreaTagMap().get(field)));
		}
		return resultMap;
	}
	/**
	...
	public String getContentBrief(String orginHtml) {
		...
		if (StringUtils.isNotBlank(orginHtml)) {
			...
			if(CollectionUtils.isEmpty(briefAreafilter.getFetchAreaTagMap())){//如果摘要采集参数为空,则提取内容中指定字数字符作为摘要信息
	...
	public List<CrawlLinkURI> getPaginationList(String orginHtml,CrawlURI parentCrawlURI) {
		...
		if(!CollectionUtils.isEmpty(paginationAreafilter.getFetchAreaTagMap())){
	...
	public CrawlLinkURI getCommentIndexUrl(String orginHtml,CrawlURI parentCrawlURI){
		...
		if(!CollectionUtils.isEmpty(commentIndexAreafilter.getFetchAreaTagMap())){
	...
	public List<CrawlLinkURI> getCommentUrlList(String orginHtml,CrawlURI parentCrawlURI) {
		...
		if(!CollectionUtils.isEmpty(commentLinkAreafilter.getFetchAreaTagMap())){
